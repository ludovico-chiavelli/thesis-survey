...
unfilterd_corpus = Path('/home/nuvolari/GitHub/thesis-survey/final-fullcorpus/final_corpus_unfiltered_texts.csv').resolve()
...
for index, row in tqdm.tqdm(corpus_df.iterrows(), total=corpus_df.shape[0]):
    for word in wordlist:
        if word in row['HUMAN_TEXT'].lower().split():
